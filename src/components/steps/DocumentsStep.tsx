...
export default function DocumentsStep() {
  ...
  const MAX_FILE_SIZE_MB = 5; // 5MB maximum file size (increased from 4MB)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check image files limit (max 4, increased from 2)
            ...
            <div className="bg-white/10 rounded p-3 mb-4 border border-white/20 flex items-center gap-2 text-sm text-white/80">
              ...
              <span>
                {hasPdf 
                  ...
                  : hasImages 
                    ? `You've uploaded ${imageCount} of maximum 4 image files`
                    : "Choose either 1 PDF file (max 5MB) or up to 4 image files (max 5MB each)"}
              </span>
            </div>
            <input
              type="file"
